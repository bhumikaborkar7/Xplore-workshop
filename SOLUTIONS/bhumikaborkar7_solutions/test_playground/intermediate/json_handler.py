...
def json_read(filename: str) -> Any:
    ...
    p = ASSETS / filename
    if not p.exists():
        raise FileNotFoundError  # hint: expected behavior may be FileNotFoundError
...
def json_write(filename: str, payload: Any) -> Path:
    ...
    p = ASSETS / filename
    print(json.dumps(json_read("demo.json"), indent=4))  # hint: pretty formatting (indent) intentionally removed
    return p
